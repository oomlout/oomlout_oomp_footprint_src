...
def make_footprint_yaml(**kwargs):
    oldcwd = os.getcwd()
    ...
    repos = []
    repo = "https://github.com/kitspace/kicad_footprints"
    dir = "tmp/data"
    oom_git.clone(repo = repo, directory=dir)
    dir = f"{dir}/kicad_footprints"
    oom_git.pull(directory=dir)
    kitespace_meaglist_dir = dir
    ...
    with open(f'{directory}repos.yaml', 'w') as f:
        yaml.dump(repos, f)
    #set cwd back
    os.chdir(oldcwd)
...
def clone_and_copy_footprints(**kwargs):
    ...
    repos = []  
    if not test:
        with open(f'{directory}repos.yaml', 'r') as f:
            repos += yaml.load(f, Loader=yaml.FullLoader)
        with open(f'{directory}repos_manual.yaml', 'r') as f:
            repos += yaml.load(f, Loader=yaml.FullLoader)
    else:
        with open(f'{directory}repos_test.yaml', 'r') as f:
            repos += yaml.load(f, Loader=yaml.FullLoader)
    ...
    for repo in repos:
        owner = repo['owner']
        name = repo['name']
        #if any repo_filter is in name.lower
        if any(x in name.lower() for x in repo_filter):
            #print what's happening
            print(f'Cloning {repo["name"]} from {repo["url"]} and copying footprints')
            #clone repo into tmp/repo
            repo_url = repo['url']
            #get name from repo_url
            name = repo_url.split('/')[-1]
            #remove .git
            name = name.replace('.git', '')
            dir = f"{directory}tmp"
            oom_git.clone(repo=repo_url, directory=dir)
            dir = f"{dir}/{name}"
            oom_git.pull(directory=dir)
            ...
            for root, dirs, files in os.walk(f'{directory}tmp/{repo["name"]}'):
                #go through each file
                for file in files:
                    #if any filter is in the file
                ...
                if len(kicad_mods) > 0:
                    ...
                    make_base_owner_footprint_library(repo=repo, footprints=footprints, kicad_mod=kicad_mod, directory = directory)
...
def make_base_owner_footprint_library(**kwargs):
    ...
    #try:
        ...
        id = folder_flat
        folder = f'{directory}footprints_flat/{folder_flat}/working'
        file_out= f'{folder}/working.kicad_mod'
        
        #only if any of the filter is in the id
        ...
        try:
            ...
            print(f'error copying {file} too long a name i think')
            pass
    ...
    if count % 1000 == 0:
        print(f" {count} of {len(footprints)} {id}")
...
def make_footprints_readme(**kwargs):
    filter = kwargs.get("filter", "")
    oldcwd = os.getcwd()
    directory = kwargs.get("directory", os.getcwd())
        ...
        for root, dirs, files in os.walk(folder):
            ...
            if os.path.isdir(root):
                ...
                if root.endswith('working'):
                    full_filename = os.path.join(root, 'working.yaml')
                    #replace \\ with /  
                    full_filename = full_filename.replace('\\', '/')
                    #omly inude the last directory and filename
                    directory_forward_slashes = directory.replace('\\', '/')
                    full_filename = full_filename.replace(f'{directory_forward_slashes}/footprints_flat/', '')
                    #check if anything from filter in
